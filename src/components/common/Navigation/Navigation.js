...
import { endSessionAPI } from '../../../services/queryAPI';
...
const Navigation = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Get sessionTimestamp and sessionStart from sessionStorage
      const sessionTimestamp = sessionStorage.getItem("chatSessionTimestamp");
      const sessionStart = sessionStorage.getItem("chatSessionStart");
      // Trigger end session event if possible (do not block logout if it fails)
      if (userId && sessionTimestamp && sessionStart) {
        try {
          const endSessionResponse = await endSessionAPI(
            userId,
            sessionTimestamp
          );
          ...
          console.error(
            "Failed to end session or upload chat log metadata:",
            e
          );
        }
      }
